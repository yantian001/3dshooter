		...
		public void Init(){
			...
			for(int i=0; i<perkList.Count; i++){
				if(purchasedIDList.Contains(perkList[i].ID)){
					if(perkList[i].type==_PerkType.NewTower || perkList[i].type==_PerkType.NewAbility){
		...
		public string _PurchasePerk(Perk perk, bool useRsc=true){
			...
			if(perk.type==_PerkType.NewTower){ 
				...
				for(int i=0; i<towerList.Count; i++){
					if(towerList[i].prefabID==perk.itemIDList[0]) BuildManager.AddNewTower(towerList[i]);
				}
			}
			else if(perk.type==_PerkType.NewAbility){
				List<Ability> abilityList=AbilityDB.Load();
				for(int i=0; i<abilityList.Count; i++){
					if(abilityList[i].ID==perk.itemIDList[0]) AbilityManager.AddNewAbility(abilityList[i]);
				}
			}
			else if(perk.type==_PerkType.NewFPSWeapon){
				List<FPSWeapon> FPSWeaponList=FPSWeaponDB.Load();
				for(int i=0; i<FPSWeaponList.Count; i++){
					if(FPSWeaponList[i].prefabID==perk.itemIDList[0]) {}//FPSControl.AddNewFPSWeapon(FPSWeaponList[i]);
				}
			}
				...
				Debug.Log("energy cap increased by "+perk.value);
